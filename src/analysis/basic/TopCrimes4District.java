...
public class TopCrimes4District implements java.io.Serializable {
	...
	public void run() {
		...
		JavaPairRDD<String, Integer> districtAndCrime = documents.flatMapToPair(
				t-> {
					...
					if (!(district instanceof String)){
						...
						typeOfCrime = String.valueOf(t._2.get("Primary Type"));
					} else {
						district2 = (String)district;
						typeOfCrime = ((String)t._2.get("Primary Type")).trim();
					}

					if (district2 != null && district2.length() > 0 && typeOfCrime != null && typeOfCrime.length() > 0) {	
						key = district2+"|"+typeOfCrime;
		...
		JavaPairRDD<String, Integer> count = districtAndCrime.reduceByKey(
				(a, b) -> a + b);
		...
		outputConfig.set("mongo.output.uri",
				"mongodb://localhost:27017/bigData.topCrimes4District");
