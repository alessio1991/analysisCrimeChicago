...
public class TopLocation4District implements java.io.Serializable {
	...
	public void run() {
		...
		//Arrange the keys for the count --> every key is made from districtNumber+location so that it is possible to do the count with these fields together
		JavaPairRDD<String, Integer> districtAndLocation = documents.flatMapToPair(
				t-> {					
					...
					if (!(district instanceof String)){
						district2 = String.valueOf(((Number)(t._2.get("District"))).intValue());
					} else {
						district2 = (String)district;
					}

					if (district2 != null && district2.length() > 0 && location != null && location.length() > 0) {	
		...
		//count the crime events in every location for every district
		...
		outputConfig.set("mongo.output.uri",
				"mongodb://localhost:27017/bigData.topLocation4District");
