...
public class TopCrimes4Year implements java.io.Serializable {
	...
	public void run() {
		...
		//Arrange the keys for the count --> every key is made from year+typeOfCrime so that it is possible to do the count with these fields together
		JavaPairRDD<String, Integer> yearOfCrime = documents.flatMapToPair(
				t-> {					
		...
		//count the type of crime for every year
		JavaPairRDD<String, Integer> count = yearOfCrime.reduceByKey(
				(a, b) -> a + b);
		...
		outputConfig.set("mongo.output.uri",
				"mongodb://localhost:27017/bigData.topCrimes4Year");
