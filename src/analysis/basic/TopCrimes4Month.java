...
public class TopCrimes4Month implements java.io.Serializable {
	...
	public void run() {
		...
		//Arrange the keys for the count --> every key is made from year/month+typeOfCrime so that it is possible to do the count with these fields together
		JavaPairRDD<String, Integer> monthOfCrime = documents.flatMapToPair(
				t-> {					
					List<Tuple2<String,Integer>> crimesInDistrict = new ArrayList<Tuple2<String,Integer>>();
					
					String dateTimestamp = ((String)t._2.get("Date")).trim();
					String month = dateTimestamp.substring(0, dateTimestamp.indexOf('/'));
					
					Object year = t._2.get("Year");
					String yearFinal = "";
					if (!(year instanceof String)){
						yearFinal = String.valueOf(((Number)(t._2.get("Year"))).intValue());
					} else {
						yearFinal = (String)year;
					} 
					
					String yearMonth = yearFinal+"/"+month;
		...
		//count the type of crime for every month
		JavaPairRDD<String, Integer> count = monthOfCrime.reduceByKey(
				(a, b) -> a + b);
	    ...
	    JavaPairRDD<Object, BSONObject> finalOrderedTopCrimes = bsonOrdered.flatMapToPair(
				t -> {
					...
					bson.put("YearAndMonth", t._1);
					bson.put("CrimesInOrder", t._2);
		...
		outputConfig.set("mongo.output.uri",
				"mongodb://localhost:27017/bigData.topCrimes4Month");
